...
## load data again
Data <- read.spss("C:/Social class analysis/Sociale Index enquete 2016 arbeidss recode v3.sav", use.value.labels = FALSE, to.data.frame = TRUE)
...
### select variables
Data6 <- Data %>% select(werkstatus3, Income_imprec, Income_imp2rec,Income_imp3rec,Income_imp4rec,Income_imp5rec,Opleid_new, socialsupport_new,contactfam_nieuw,contactvriend_nieuw,cultuurvar2_new, huiseigenaar,MEAN_WOZ_WAARDE_2,MEAN_WOZ_WAARDE_10)

####Recodes
Data6$Income_rev <- 6 - Data6$Income_imprec
Data6$Income_rev2 <- 6 - Data6$Income_imp2rec 
Data6$Income_rev3 <- 6 - Data6$Income_imp3rec 
Data6$Income_rev4 <- 6 - Data6$Income_imp4rec 
Data6$Income_rev5 <- 6 - Data6$Income_imp5rec 
Data6$Opleid_new_rev <- 11 - Data6$Opleid_new
Data6$socialsupport_rev <- 5 - Data6$socialsupport_new
Data6$contactfam_rev <- 5 - Data6$contactfam_nieuw
Data6$contactvriend_rev <- 5 - Data6$contactvriend_nieuw
Data6$cultuurvar2_rev <- 5 - Data6$cultuurvar2_new
...
## classes and werkstatus
lcmodel9 <- melt(lc9impEXP$probs, level = 2)
lcmodel9subs <- subset(lcmodel9, L2 == "werkstatus3")
...
probs.start <- lc9impEXP$probs.start
new.probs.start <- poLCA.reorder(probs.start, c(2,9,1,7,6,5,3,8,4))
lc9impEXPher <- poLCA(formimp,Data, nclass = 9, probs.start = new.probs.start, na.rm = FALSE)

### classification table.

posteriors <- data.frame(lc9impEXPher$posterior, predclass = lc9impEXPher$predclass)
classification_table <- ddply(posteriors, .(predclass), function(x) colSums(x[,1:9]))

round(classification_table,1)


###################################################################
############ final edits for presentation paper ###################

Data$Income_revDEF <- 6 - Data$Income_imprec
Data$Opleid_new_revDEF <- 9 - Data$Opleid_new
Data$Opleid_new_revDEF[Data$Opleid_new_revDEF==4] <- 6
Data$Opleid_new_revDEF[Data$Opleid_new_revDEF==5] <- 4
Data$Opleid_new_revDEF[Data$Opleid_new_revDEF==6] <- 5
Data$Opleid_new_revDEF[Data$Opleid_new_revDEF==7] <- 6
Data$Opleid_new_revDEF[Data$Opleid_new_revDEF==8] <- 7
Data$Opleid_new_revDEF[Data$Opleid_new_revDEF==0|Data$Opleid_new_revDEF==-1] <- 8
Data$socialsupport_revDEF <- 4 - Data$socialsupport_new
Data$socialsupport_revDEF[Data$socialsupport_revDEF==0] <- 4
Data$contactfam_revDEF <- 5 - Data$contactfam_nieuw
Data$contactvriend_revDEF <- 5 - Data$contactvriend_nieuw
Data$cultuurvar2_revDEF <- 5 - Data$cultuurvar2_new
Data$werkstatus3[Data$werkstatus3==10] <- 9

formimpDEF <- with(Data, cbind(werkstatus3, Income_revDEF, Opleid_new_revDEF, socialsupport_revDEF,contactfam_revDEF,contactvriend_revDEF,cultuurvar2_revDEF)~1)

lc9DEF <- poLCA(formimpDEF, Data, nclass = 9, maxiter = 15000, tol=1e-5, na.rm = FALSE, nrep = 10)

probs.start <- lc9DEF$probs.start
new.probs.start <- poLCA.reorder(probs.start, c(1,8,5,4,9,7,3,2,6))
lc9DEFher <- poLCA(formimpDEF,Data, nclass = 9, probs.start = new.probs.start, na.rm = FALSE)

### plots ###
## classes and werkstatus
lcmodel9 <- melt(lc9DEFher$probs, level = 2)
lcmodel9subs <- subset(lcmodel9, L2 == "werkstatus3")
plot9 <- ggplot(lcmodel9subs, aes(x= Var1, y=value, fill=Var2))
plot9 <- plot9 + geom_bar(stat = "identity", position = "stack", colour = "black")
paletwork <- c("#8dd3c7","#ffffb3","#bebada","#fb8072","#80b1d3","#fdb462","#b3de69","#fccde5","#f0f0f0")
plot9 <- plot9 + scale_y_continuous("Conditional response probabilities")
plot9 <- plot9 + scale_x_discrete("Social classes", labels = c("EUC","SUC","SMC","RMC","SLMC","VE","PC","SC","SR"))
plot9 <- plot9 + scale_fill_manual(values = paletwork,labels=c("Full-time","Part-time","Unemployed","Incapacity for work","Welfare benefits","Retired","Housewife/husband","Student","Missing")) + labs(fill="Employment status") + theme_bw()
plot9

ggsave("C:/Social class analysis/plot_employment.tiff", width = 6, height = 4, dpi = 300)

#class and income
lcmodel9subs2 <- subset(lcmodel9, L2 == "Income_revDEF")
plot9a <- ggplot(lcmodel9subs2, aes(x= Var1, y=value, fill=Var2))
plot9a <- plot9a + geom_bar(stat = "identity",colour = "black")
paleta <- c("#252525","#636363","#969696","#cccccc","#f7f7f7")
levelsincomea <- c(">3,350","2,000 - 3,350","1,500 - 2,000","1,000 - 1,500","<1,1000")
plot9a <- plot9a + scale_y_continuous("Conditional response probabilities")
plot9a <- plot9a + scale_x_discrete("Social classes", labels = c("EUC","SUC","SMC","RMC","SLMC","VE","PC","SC","SR"))
plot9a <- plot9a + scale_fill_manual(values = paleta, labels=levelsincomea) + labs(fill="Household income") +theme_bw()
plot9a

ggsave("C:/Social class analysis/plot_income.tiff", width = 6, height = 4, dpi = 300)


#class and socialsupport
lcmodel9subs3 <- subset(lcmodel9, L2 == "socialsupport_revDEF")
plot9b <- ggplot(lcmodel9subs3, aes(x= Var1, y=value, fill=Var2))
plot9b <- plot9b + geom_bar(stat = "identity", position = "stack", colour = "black")
socialsupportanswer9b <- c( "(totally) agree","neutral","(totally) disagree","Missing")
palet9b <- c("#636363","#bdbdbd","#f7f7f7","#f0f0f0")
plot9b <- plot9b + scale_y_continuous("Conditional response probabilities")
plot9b <- plot9b + scale_x_discrete("Social classes", labels = c("EUC","SUC","SMC","RMC","SLMC","VE","PC","SC","SR"))
plot9b <- plot9b + scale_fill_manual(values = palet9b, labels=socialsupportanswer9b) + labs(fill="Social support") +theme_bw()
plot9b

ggsave("C:/Social class analysis/plot_socialsupport.tiff", width = 6, height = 4, dpi = 300)

#class and contact
lcmodel9subs4 <- subset(lcmodel9, L2 == "contactfam_revDEF" | L2 == "contactvriend_revDEF")
plot9c <- ggplot(lcmodel9subs4, aes(x= Var1, y=value, fill=Var2))
plot9c <- plot9c + geom_bar(stat = "identity", position = "stack", colour = "black")
namesfacet <- c('contactfam_revDEF' = "Family", 'contactvriend_revDEF' = "Friends")
plot9c <- plot9c + facet_grid(. ~ L2, labeller = as_labeller(namesfacet))
contactanswer9c <- c("Almost daily","> Once a week","> Once a month","< once a month / never")
plot9c <- plot9c + scale_y_continuous("Conditional response probabilities")
plot9c <- plot9c + scale_x_discrete("Social classes", labels = c("EUC","SUC","SMC","RMC","SLMC","VE","PC","SC","SR"))
palet9c <- c("#525252","#969696","#cccccc","#f7f7f7")
plot9c <- plot9c + scale_fill_manual(values = palet9c,labels=contactanswer9c) + labs(fill="Contact frequency") + theme_bw() + theme(axis.text.x=element_text(angle=90, hjust=1, vjust=.5))
plot9c

ggsave("C:/Social class analysis/plot_contact.tiff", width = 6, height = 4, dpi = 300)


# class and culture
lcmodel9subs5 <- subset(lcmodel9, L2 == "cultuurvar2_revDEF")
plot9d <- ggplot(lcmodel9subs5, aes(x= Var1, y=value, fill=Var2))
plot9d <- plot9d + geom_bar(stat = "identity", position = "stack", colour = "black")
plot9d <- plot9d + scale_y_continuous("Conditional response probabilities")
plot9d <- plot9d + scale_x_discrete("Social classes", labels = c("EUC","SUC","SMC","RMC","SLMC","VE","PC","SC","SR"))
cultuurvaranswer9d <- c("> Once a week","> Once a month","< Once a month","Never")
plot9d <- plot9d + scale_fill_manual(values = palet9c, labels=cultuurvaranswer9d) + labs(fill="Culture visit") +theme_bw()
plot9d

ggsave("C:/Social class analysis/plot_culture.tiff", width = 6, height = 4, dpi = 300)


## class and education
lcmodel9subs6 <- subset(lcmodel9, L2 == "Opleid_new_revDEF")
plot9e <- ggplot(lcmodel9subs6, aes(x= Var1, y=value, fill=Var2))
plot9e <- plot9e + geom_bar(stat = "identity", position = "stack", colour = "black")
plot9e <- plot9e + scale_y_continuous("Conditional response probabilities")
plot9e <- plot9e + scale_x_discrete("Social classes", labels = c("EUC","SUC","SMC","RMC","SLMC","VE","PC","SC","SR"))
opleid9e <- c("University","Higher professional","Senior general secondary","Senior secondary vocational","Junior general secondary","Junior secondary vocational","Primary / none", "Missing")
palet9e <- c("#252525","#525252","#737373","#969696","#bdbdbd","#d9d9d9","#f7f7f7","#f0f0f0")
plot9e <- plot9e + scale_fill_manual(values = palet9e, labels=opleid9e) + labs(fill="Education") + theme_bw()
plot9e

ggsave("C:/Social class analysis/plot_educ.tiff", width = 6, height = 4, dpi = 300)


### classification_table

posteriors <- data.frame(lc9DEFher$posterior, predclass = lc9DEFher$predclass)
classification_table <- ddply(posteriors, .(predclass), function(x) colSums(x[,1:9]))

round(classification_table,1)

####### Repeating analyses with imputations and creating new datset

###original imputation
Data$assclass <-  lc9DEF$predclass
...
## write data as excel and open in SPSS
write.xlsx(DataEXP, "C:/Social class analysis/DEF/excelimp.xlsx", sheetName="Sheet1")

####imputation2
Data$Income_revDEF2 <- 6 - Data$Income_imp2rec

formimpDEF2 <- with(Data, cbind(werkstatus3, Income_revDEF2, Opleid_new_revDEF, socialsupport_revDEF,contactfam_revDEF,contactvriend_revDEF,cultuurvar2_revDEF)~1)
lc9DEF2 <- poLCA(formimpDEF2, Data, nclass = 9, maxiter = 15000, tol=1e-5, na.rm = FALSE, nrep = 10)

Data$assclass2 <- lc9DEF2$predclass
...
write.xlsx(DataEXP2, "C:/Social class analysis/DEF/excelimp2.xlsx", sheetName="Sheet1")
...
####imputation3
Data$Income_revDEF3 <- 6 - Data$Income_imp3rec

formimpDEF3 <- with(Data, cbind(werkstatus3, Income_revDEF3, Opleid_new_revDEF, socialsupport_revDEF,contactfam_revDEF,contactvriend_revDEF,cultuurvar2_revDEF)~1)
lc9DEF3 <- poLCA(formimpDEF3, Data, nclass = 9, maxiter = 15000, tol=1e-5, na.rm = FALSE, nrep = 10)

Data$assclass3 <- lc9DEF3$predclass
...
write.xlsx(DataEXP3, "C:/Social class analysis/DEF/excelimp3.xlsx", sheetName="Sheet1")

####imputation4
Data$Income_revDEF4 <- 6 - Data$Income_imp4rec

formimpDEF4 <- with(Data, cbind(werkstatus3, Income_revDEF4, Opleid_new_revDEF, socialsupport_revDEF,contactfam_revDEF,contactvriend_revDEF,cultuurvar2_revDEF)~1)
lc9DEF4 <- poLCA(formimpDEF4, Data, nclass = 9, maxiter = 15000, tol=1e-5, na.rm = FALSE, nrep = 10)

Data$assclass4 <- lc9DEF4$predclass
...
write.xlsx(DataEXP4, "C:/Social class analysis/DEF/excelimp4.xlsx", sheetName="Sheet1")

####imputation5
Data$Income_revDEF5 <- 6 - Data$Income_imp5rec

formimpDEF5 <- with(Data, cbind(werkstatus3, Income_revDEF5, Opleid_new_revDEF, socialsupport_revDEF,contactfam_revDEF,contactvriend_revDEF,cultuurvar2_revDEF)~1)
lc9DEF5 <- poLCA(formimpDEF5, Data, nclass = 9, maxiter = 15000, tol=1e-5, na.rm = FALSE, nrep = 10)

Data$assclass5 <- lc9DEF5$predclass

DataEXP5 <- Data %>% select(werkstatus3, Income_imp5rec, Opleid_new, socialsupport_new,contactfam_nieuw,contactvriend_nieuw,cultuurvar2_new,assclass5,buurt,v1,v2,hhtype,etnic,Tenure, Wijk71)

DataEXP5 <- inner_join(DataEXP5, Context2015)

DataEXP5 <- DataEXP5 %>% rename(werkst = werkstatus3 ,Income= Income_imp5rec ,Opleid= Opleid_new , socsup=socialsupport_new ,contfam=contactfam_nieuw ,contvrie=contactvriend_nieuw, cultpart=cultuurvar2_new, leeft=v1 ,gesl=v2, Dispinc = Disposable_income, P_unemp = Perc_unemployed, P_ben = Perc_benefits)

write.xlsx(DataEXP5, "C:/Social class analysis/DEF/excelimp5.xlsx", sheetName="Sheet1")
